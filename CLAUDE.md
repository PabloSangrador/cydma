...
<projects>
  ...
  Better Auth (when configured) derives its base URL per-request from reverse proxy headers (X-Forwarded-Host/Proto) via trustedProxyHeaders: true. Do NOT set baseURL in the betterAuth() config.
...
<coordination>
  ...
  3. Test backend with cURL (use relative URLs: curl http://localhost:3000/api/endpoint in dev)
